...
#Adonis2
vegan::adonis(BC_dist~ Group + Batch , permutations = 999, data = metadata, na.action = F)
```
...
#Coin. Bloking Batch.
####Wilcoxon test. Using relative abundances.
```{r Wilcox }
library("coin")
#Wilcoxon
pseq.rel <- microbiome::transform(ps_0, "compositional")
...
set.seed(321)
ps.t<- pseq.rel %>% subset_samples(Group %in% c("ccl4+rif", "ccl4")) %>% tax_glom(taxrank = 'Genus')
test.taxa <- taxa(ps.t)
pvalue_wilcoxon <- c()
foldchange <- c()
for (taxa in test.taxa) {
...
rownames(pvalues_wilcoxcoin)<- pvalues_wilcoxcoin$`names(pvalue_wilcoxon[i])`
write.csv(cbind(as.data.frame(pvalues_wilcoxcoin), as.data.frame(tax_table(ps.t))), "Wilcoxcoin_ccl4vsccl4+rif_genus_Ztransform")
```
...
```{r mvabund}
library(mvabund)
ps.t<- tax_glom(ps_0, taxrank = 'Genus')
obj <- as.data.frame(t(otu_table(ps_0)))
...
boxplot.mvabund(obj)
meanvar.plot(obj)
glm <-manyglm(obj~(metadata$Group) + metadata$Batch, family="negative.binomial")
sumglm <- summary.manyglm(glm, resamp = "montecarlo", p.uni = "adjusted")
View(sumglm[["uni.p"]])
write.csv(cbind(as.data.frame(sumglm[["uni.p"]]), as.data.frame(tax_table(ps_0))), "DAA_binomialmodel_mvabund_notransforms_genus_vsccl4")
glm2 <-manyglm(obj~(metadata1$Group)+ metadata1$Batch)
sumglm2 <- summary.manyglm(glm2, resamp = "montecarlo", p.uni = "adjusted")
View(sumglm2[["uni.p"]])
write.csv(cbind(as.data.frame(sumglm2[["uni.p"]]), as.data.frame(tax_table(ps_0))), "DAA_binomialmodel_mvabund_notransforms_genus")
```
