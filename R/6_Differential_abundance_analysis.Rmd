...
#Coin. Bloking Batch.
####Wilcoxon test. Using non-transformed data(relative abundances).
```{r Wilcox }
library("coin")
#Wilcoxon
pseq.rel <- microbiome::transform(ps_0, "compositional")
...
write.csv(cbind(as.data.frame(pvalues_wilcoxcoin), as.data.frame(tax_table(ps.t))), "Wilcoxcoin_ccl4vsccl4+rif_genus")
```
...
library("coin")
#Wilcoxon
pseq.rel <- microbiome::transform(ps_0, "Z")
otu <- abundances(pseq.rel)
hist(otu, 113)
...
boxplot.mvabund(obj)
meanvar.plot(obj)
glm <-manyglm(obj~(metadata$Treated_CCL) + (metadata$Antibiotic) + metadata$Batch)
sumglm <- summary.manyglm(glm, resamp = "montecarlo", p.uni = "adjusted")
glm2 <-manyglm(obj~(metadata1$Group)+ metadata$Batch)
