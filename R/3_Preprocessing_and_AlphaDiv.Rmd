...
theme_classic() + theme(strip.background = element_blank(), axis.text.x.bottom = element_text(angle = -90), axis.text.x = element_text(size = 16), axis.title.y = element_text(size = 16), legend.position = "none", text = element_text(size = 18)) +
      geom_signif(stat="identity",
               data=data.frame(x=c(1, 2, 3), xend=c(2, 3, 4),
                               y=c(78, 80, 82), annotation=c("NS", " NS ", "*")),
               aes(x=x,xend=xend, y=y, yend=y, annotation=annotation)) +
...
theme_classic() + theme(strip.background = element_blank(), axis.text.x.bottom = element_text(angle = -90), axis.text.x = element_text(size = 16), axis.title.y = element_text(size = 16)) +
    ...
    geom_signif(comparisons=list(c("ctr", "ccl4+rif")), annotations="**",
                y_position = 9.80, tip_length = 0, vjust=0.4) + theme(
        axis.text.x = element_text(size = 16), axis.title.y = element_text(size = 16), legend.position = "none", text = element_text(size = 18))

newSTorder = c("ctr", "ctr+rif", "ccl4", "ccl4+rif")
pFisher$data$Group <- as.character(pFisher$data$Group)
pFisher$data$Group <- factor(pFisher$data$Group, levels=newSTorder)
pFisher
ggarrange(pObserved, pShannon, pChao, pFisher,common.legend = TRUE, legend="bottom",nrow = 1)
```
...
# turn the otu_table into a data.frame
otu_export <- as.data.frame(t(otu_table(ps.rel2)))
tmp <- names(otu_export)
