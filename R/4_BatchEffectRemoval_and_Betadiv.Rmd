...
#Raw data
rawDada2_Species <- as.data.frame(otu_table(ps_1))
...
OTU = otu_table(OTU_data_0, taxa_are_rows = FALSE)
TAX = tax_table(NewTAX, taxa_are_rows = FALSE)
SAMPLE <- sample_data(metadata)
...
# merge the data
ps_0 <- phyloseq(OTU, TAX, SAMPLE)
ps_0

 
  tax_table(ps_0)[, colnames(tax_table(ps_0))] <- gsub(tax_table(ps_0)[, colnames(tax_table(ps_0))],  pattern = "[a-z]__", replacement = "") 
```